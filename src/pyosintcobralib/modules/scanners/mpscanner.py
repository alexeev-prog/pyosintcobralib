...
class MeterpreterScanner:
    ...
    def _search_process_with_dll(self, dll: str) -> None:
        output_tasklist = subprocess.check_output(
            f"{CMD_TASKLIST_COMMAND} {dll}", shell=True
        )
        ...
        for l_socket in local_sockets:
            ...
            if int(l_port) >= 49152:
                if l_ip != "127.0.0.1":
                    ...
                    data_with_suspicious_socket = scan_output.decode().split(
                        victim_socket
                    )
                    suspicious_info = re.findall(
                        REG_FOR_REMOTE_SOCKET, data_with_suspicious_socket[1]
                    )[0]
                    ...
                    print(
                        f"[-] Подключение {victim_socket} к {suspicious_socket} "
    ...
    def finding_meterpreter_sessions(self):
        ...
        return {"found": found, "processes": self._processes_with_signature}
